...
Mikrofon, Verlauf auf dem Telefon, Antworten optional vorgelesen. Sie spricht
direkt mit der Claude-API und braucht **keinen Server und keinen Zugangscode**
— nur einen API-Schlüssel, den Damaso einmal in den Einstellungen einträgt.

Das ist der Unterschied zu den beiden anderen Fassungen:
...
| **`jarvis-android/`** | **verschlüsselt auf dem Telefon** | **nur Netz** |
...
Beim ersten Start: Zahnrad oben rechts, API-Schlüssel von
[console.anthropic.com](https://console.anthropic.com) eintragen, sichern.
...
app/src/main/java/ch/damaso/jarvis/chat/
  ...
  Modell.kt         Der Aufruf an die Messages-API und das Lesen des Stroms
  Tresor.kt         Der API-Schlüssel, verschlüsselt abgelegt
...
**Keine Bibliothek für den Schlüssel.** `Tresor.kt` verschlüsselt ihn mit
...
**Direkt zur API, ohne Server dazwischen.** Im Browser wäre ein Schlüssel für
jeden Besucher lesbar — deshalb steht bei der Web-Fassung eine Netlify-Function
davor. Auf Damasos eigenem Telefon ist das anders: der Schlüssel liegt dort
ohnehin, und ein Zwischenserver würde nur Netz und Geld kosten.
...
tatsächlich gesendet wird, ob der Text stückweise ankommt, ob eine abgerissene
Antwort als abgerissen erkannt wird, ob bei einem abgelehnten Schlüssel die
zweite Form der Zugangsdaten probiert wird. Läuft bei jedem Push mit.
